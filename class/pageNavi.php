...
class pageNavi
{
    ...
    public $current_page_message =''; //　件中、n件～m件を表示といったメッセージ

    // コンストラクタ
    //$navi_max    10,11,12,13などリンクを表示する件数4だったら4件分
    //$page_row    一ページ当たりの表示件数 
    public function __construct($now_page, $all_row, $page_row, $func_name, $navi_max ="", $urlParam = '', $display_number = true)
    {
        ...
        if (!preg_match("/^[[:digit:]]+$/", $now_page) || $now_page < 1 || strlen($now_page) == 0) {
            ...
            $this->now_page = htmlspecialchars($now_page, ENT_QUOTES);
        }
        ...
        //(ページ当たりの)最終レコード
        $this->last_row     = ( $this->start_row + $page_row > $all_row ) ? $all_row : $this->start_row + $page_row ;
        ...
        if ($all_row > 1) {
            ...
            $this->current_page_message =  $all_row . '件中、' .  ( $this->start_row + 1 ) . '件目～' . $this->last_row . '件目を表示';
        
        } else {

            $this->current_page_message =  '1件を表示';
