...
class utility {
    ...
    function sfInitInstall() {
        ...
        if (!defined('ECCUBE_INSTALL')) {
            ...
            if( strpos('/install/', $phpself) === false ) {
                ...
                $install_url = self::searchInstallerPath($path);
        ...
        if(file_exists($path)) {
            self::sfErrorHeader('&gt;&gt; /install/' . DIR_INDEX_FILE . ' は、インストール完了後にファイルを削除してください。');
        }
    }
    ...
    function searchInstallerPath($path) {
        ...
        $resources = fopen(self::getRealURL($installer_url), 'r');
        if ($resources === false) {
            $installer_url = self::searchInstallerPath($path . '../');
        }
    ...
    function sfIsSuccess($objSess, $disp_error = true) {
        ...
        if ( empty($_SERVER['HTTP_REFERER']) ) {
            ...
            $domain  = self::sfIsHTTPS() ? HTTPS_URL : HTTP_URL;
            $pattern = sprintf('|^%s.*|', $domain);
            $referer = $_SERVER['HTTP_REFERER'];

            // 管理画面から以外の遷移の場合はエラー画面を表示
            if (!preg_match($pattern, $referer)) {
                if ($disp_error) self::sfDispError(INVALID_MOVE_ERRORR);
                return false;
            }
        }
        return true;
    ...
    function sfArrKeyValue($arrList, $keyname, $valname, $len_max = "", $keysize = "") {
        ...
        for($cnt = 0; $cnt < $max; $cnt++) {
            if($keysize != "") {
                $key = self::sfCutString($arrList[$cnt][$keyname], $keysize);
            } else {
    ...
    function sfCalcIncTax($price, $tax, $tax_rule) {
        return $price + self::sfTax($price, $tax, $tax_rule);
    }
    ...
    function sfRound($value, $pow = 0){
        ...
        // 整数且つ0出なければ桁数指定を行う
        if(self::sfIsInt($adjust) and $pow > 1){
    ...
    function sfGetClassCatCount() {
        ...
        // キーと値をセットした配列を取得
        $arrRet = self::sfArrKeyValue($arrList, 'class_id', 'count');
    ...
    function sfMakeHiddenArray($arrSrc, $arrDst = array(), $parent_key = "") {
        if(is_array($arrSrc)) {
            foreach($arrSrc as $key => $val) {
                ...
                if(is_array($val)) {
                    $arrDst = self::sfMakeHiddenArray($val, $arrDst, $keyname);
                } else {
                    $arrDst[$keyname] = $val;
                }
            }
    ...
    function sfCopyDir($src, $des, $mess = "", $override = false){
        ...
        if (is_array($fileArray)) {
            foreach( $fileArray as $key => $data_ ){
                ...
                $data=$matches[2];
                if( is_dir( $data_ ) ){
                    $mess = self::sfCopyDir( $data_.'/', $des.$data.'/', $mess);
                }else{
            ...
            if ($dh = opendir($dir)) {
                ...
                foreach($arrDir as $file) {
                    ...
                    if($file != "." && $file != "..") {
                        ...
                        $file_name = self::sfCutString($file, FILE_NAME_LEN);
                        $file_size = self::sfCutString(self::sfGetDirSize($path), FILE_NAME_LEN);
   ...
   /**
    ...
    function sfMakePluginInfoArray($file) {
        $fullPath = self::sfGetPluginFullPathByRequireFilePath($file);

        return
            array(
                // パス
                'path' => self::sfGetPluginPathByPluginFullPath($fullPath),
                // プラグイン名
                'name' => '未定義',
                // フルパス
                'fullpath' => $fullPath,
    ...
    function sfPutPluginsXml($pluginsXml) {
        ...
        if (strlen($xml) == 0) self::sfDispException();

        $return = file_put_contents(PLUGIN_REALDIR . 'plugins.xml', $pluginsXml->asXML());
        if ($return === false) self::sfDispException();
        return $return;
    }
        ...
        if ($hashpass != '') {
            if (AUTH_TYPE == 'PLAIN') {
                ...
                if (empty($salt)) {
                    ...
                    $hash = self::sfGetHashString($pass, $salt);
                }
                if($hash === $hashpass) {
                    $res = true;
                }
    ...
    function sfGetSearchPageMax($search_page_max) {
        if (self::sfIsInt($search_page_max) && $search_page_max > 0) {
